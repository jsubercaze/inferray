...
/**
 * Dedicated class for prp-trp. Does not enter any template.
 *
 *
 ...
 *         Feb. 14
 */
public class FC_PRP_TRP extends AbstractFastRule {

	private final static Logger logger = Logger.getLogger(FC_PRP_TRP.class);

	public FC_PRP_TRP(final NodeDictionary dictionary,
	...
	private int processTransitive(final LongPairArrayList rdfType) {
		...
		final long transitive = AbstractDictionary.owltransitiveProperty;
		int i = 1;
		...
		if (rdfType.getQuick(listSize - 1) < transitive) {
			if (logger.isTraceEnabled()) {
		...
		while (val < transitive && i < listSize - 1) {
			i += 2;
		...
		if (logger.isTraceEnabled()) {
			...
			logger.trace("Val " + val);
			logger.trace("Transitive " + transitive);
		}
		// Value is contained
		for (; i < listSize; i += 2) {
			val = rdfType.getQuick(i);
			...
			if (prop != AbstractDictionary.rdfssubClassOf
					&& prop != AbstractDictionary.rdfssubPropertyOf
					&& prop != AbstractDictionary.owlsameAs) {
				...
				LongPairArrayList listNew = usableTriples.getbyPredicate(p);
				if (listNew == null || listNew.isEmpty()) {

					// System.exit(-1);
					continue;
				}
				// Duplicate the second one and sort it by object
	...
	private int traverse(final LongPairArrayList list1,
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Traversing with " + list1);
			logger.trace("Traversing with " + list2);
		}
		for (int i = 0; i < list1.size(); i++) {
			...
			if (values[0] == previous && sizeLastAdd != 0) {
				...
				// This part is only valid for pd*
		...
		outputTriples.batchInsertion(p, output);
		if (logger.isTraceEnabled()) {
			logger.trace("EOT");
