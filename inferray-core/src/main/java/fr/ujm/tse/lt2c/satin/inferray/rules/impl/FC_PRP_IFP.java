...
/**
 * Dedicated class for prp-ifp. Does not enter any template.
 *
 * Costly method, requires double copy and object sort !!
 *
 *
 ...
 *         Feb. 14
 */
public class FC_PRP_IFP extends AbstractFastRule {

	private final static Logger logger = Logger.getLogger(FC_PRP_IFP.class);

	public FC_PRP_IFP(final NodeDictionary dictionary,
	...
	private int processTransitive(final LongPairArrayList rdfType) {
		...
		for (; i < listSize; i += 2) {
			...
			if (logger.isTraceEnabled()) {
				logger.trace("Prop " + p + "is transitive");

			}
			LongPairArrayList listMain = maintripleStore.getbyPredicate(p);
			if (listMain == null || listMain.isEmpty()) {
				if (logger.isTraceEnabled()) {
					logger.trace("Continue here");
				}
				// System.exit(-1);
				continue;
			}
			...
			// Prepare the lists - better have a copy and a sort than two sorts
			//			listMain = listMain.copy();
			//			listMain.objectSort();
			listMain = listMain.objectSortedCopy();
	...
	private int traverse(final LongPairArrayList list1,
		...
		for (int i = 0; i < list1.size(); i++) {
			if (logger.isTraceEnabled()) {
				logger.trace("Iterating Values " + Arrays.toString(values));
			}
