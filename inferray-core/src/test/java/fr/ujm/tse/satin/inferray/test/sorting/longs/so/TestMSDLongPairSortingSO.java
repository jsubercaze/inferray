package fr.ujm.tse.satin.inferray.test.sorting.longs.so;
...
import fr.ujm.tse.satin.reasoner.sorting.pairs.LongPair;

public class TestMSDLongPairSortingSO {

	private static final int RANGE = 20_000_000;

	// @Test
	public void test() {
		final long[] a = { 8, 3, 1, 5, 1, 3, 32, 12, 534, 7, 131072, 9, 1, 1 };
		long[] acopy = new long[a.length];
		System.arraycopy(a, 0, acopy, 0, a.length);
		List<LongPair> pair = LongPair.fromLongArray(acopy);
		Collections.sort(pair);
		long[] sorted = LongPair.toLongArray(pair);
		// MSDLongPairsOptim.sort(a, 131072);
		...
		pair = LongPair.fromLongArray(acopy);
		Collections.sort(pair);
		sorted = LongPair.toLongArray(pair);
		MSDLongPairsOptimAdaptativeSO.sort(c, c.length);
	...
	public void randomizedTest() {
		...
		for (int i = 0; i < 1_000; i++) {
			...
			final List<LongPair> pair = LongPair.fromLongArray(acopy);
			Collections.sort(pair);
			acopy = LongPair.toLongArray(pair);
			MSDLongPairsOptimAdaptativeSO.sort(a, a.length);
