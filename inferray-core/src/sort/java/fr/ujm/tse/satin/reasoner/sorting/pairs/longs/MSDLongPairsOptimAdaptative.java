...
public class MSDLongPairsOptimAdaptative {
	...
	public static void sort(final long[] a, final long range) {
		// Compute the max value
		final int d = bytesFor(range);
		final int N = a.length;
		final long[] aux = new long[N];
		sortKey(a, 0, N - 1, d, aux);
	}
	...
	private static int bytesFor(final long range) {
		return (int) Math.floor(Long.numberOfLeadingZeros(range) / 8D);
	}
